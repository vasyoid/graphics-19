...
MarchResult unite_smooth(MarchResult a, MarchResult b, float k) {
    ...
    return MarchResult(mix(b.dist, a.dist, h) - k * h * (1.0 - h), mix(b.color, a.color, h) - k * h * (1.0 - h), 0);
}

MarchResult intersect_smooth(MarchResult a, MarchResult b, float k) {
    float h = clamp(0.5 + 0.5 * (b.dist - a.dist) / k, 0.0, 1.0);
    return MarchResult(mix(a.dist, b.dist, h) + k * h * (1.0 - h), mix(a.color, b.color, h) + k * h * (1.0 - h), 0);
}
...
MarchResult simple_csg(vec3 p) {
    ...
    return unite(
        subtract(
            intersect(cube(p, pos, 0.5, COLOR_WHITE, 0), sphere(p, pos, 0.7, COLOR_RED, 0)),
            cylinder(p, pos, 0.2, 0.6, COLOR_GREEN, 0)
        ),
        unite(sphere(p, pos + offset, 0.25, COLOR_BLUE, 0), sphere(p, pos - offset, 0.25, COLOR_BLUE, 0))
    );
}
...
MarchResult scene(vec3 p) {
    ...
    return unite(unite(simple, smth), unite(old, ref));
}
...
MarchResult ray_marching(vec3 eye, vec3 dir) {
    ...
    for (int i = 0; i < maxSteps; ++i) {
        ...
        if (marchResult.dist < EPS) {
            return MarchResult(depth, marchResult.color, marchResult.reflect);
        }
        depth += marchResult.dist;
        if (depth >= FAR) {
            return MarchResult(FAR, COLOR_BLACK, 0);
        }
    }
    return MarchResult(FAR, COLOR_BLACK, 0);
}
